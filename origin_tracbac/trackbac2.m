...
tic
close all
%path='G:\HRtrackJD\20181122\H2O2\';
%cd(path);
file = dir([path '*.avi']);
...
for oo = 1:length(file)
    ...
    parfor index = 1:nframes
        ...
        %     level = graythresh(f2);
        bw = im2bw(f2, 0.5); %Set the threshold automatically or manually
        se = strel('disk', 4);
        bw = imopen(bw, se);
        bw = imclose(bw, se);
        %     bw = bwareaopen(bw,3);
    ...
    for i = min(frame):max(frame) - 1% loop over all frame(i),frame(i+1) pairs.
        ...
        %   screen the distance of the displacement is under the threshold
    ...
    for i = 1:lastlabel% reassemble beadlabel into a structured array 'tracks' containing all info
        ...
        tracks(i).boundingbox = boundingbox(:, baci); %bounding box 是个向量（？
